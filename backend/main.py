...
# ============================================================
# PRODUCTS — each seller manages only their own catalog;
# everyone (including guests) can browse the full combined shelf.
# ============================================================
...
def create_product(
    ...
    existing = db.query(model.Product).filter(
        model.Product.seller_id == admin.id,
        model.Product.name == product.name,
    ).first()
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You already have a product with this name")
...
def get_all_products(db: Session = Depends(get_db)):
    """Public storefront feed — every seller's products, newest first."""
    return db.query(model.Product).order_by(model.Product.id.desc()).all()


@api.get("/product/mine", response_model=List[schema.CreateProductResponse])
def get_my_products(
    db: Session = Depends(get_db),
    admin: model.User = Depends(auth.get_current_admin),
):
    """Seller dashboard feed — only this seller's own products."""
    return (
        db.query(model.Product)
        .filter(model.Product.seller_id == admin.id)
...
def delete_product(
    ...
    if db_product.seller_id != admin.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You can only remove your own products")
    db.delete(db_product)
    db.commit()


# ============================================================
# ORDERS — buyers place orders; each seller sees only the orders
# placed against their own products, never another seller's.
# ============================================================
...
def get_orders(
    ...
    if current_user.role == "customer":
        ...
        # a seller only ever sees orders placed against their own products
        query = query.filter(model.Order.seller_id == current_user.id)
...
def ship_order(
    ...
    db_order = db.query(model.Order).filter(
        model.Order.id == order_id,
        model.Order.seller_id == admin.id,
    ).first()
    if not db_order:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Order not found")
    if db_order.status != "placed":
